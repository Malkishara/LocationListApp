package com.example.locationlistapp
...
class Sql(context: Context):SQLiteOpenHelper(context,DATABASE_NAME,null,DATABASE_VERSION){
    ...
    companion object{
        ...
        private const val DATABASE_NAME="location"
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createTblLocation =("CREATE TABLE " + TBL_LOCATION + "( "
                + ID + " INTEGER PRIMARY KEY," + LOC + " TEXT, " + LATI + " TEXT, " + LON + " TEXT "+ ")")
    ...
    fun insertLocation(loc :LocationModel): Long {
        ...
        contentValues.put(LOC,loc.loc)
        contentValues.put(LON,loc.longitude)
    ...
    fun getAllLocations():ArrayList<LocationModel> {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val lo =
                    LocationModel(id = id, loc = loc, latitude = latitude, longitude = longitude)
                locList.add(lo)
